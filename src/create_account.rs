...
pub fn create_account(root: &Path, address: &str) -> std::io::Result<(Identity, Key)> {
...
mod tests {
    ...
    fn create_account_writes_identity_file() {
        in_test_dir("ark_create_account_test", |temp_dir| {
            let (_, secret_key) = create_account(temp_dir, "gyan@example.com:8080").unwrap();
            let identity_path = temp_dir.join("ark/gyan/.ark/identity.json");
    ...
    fn create_account_writes_identity_key_file() {
        in_test_dir("ark_create_account_test", |temp_dir| {
            let (_, secret_key) = create_account(temp_dir, "gyan@example.com").unwrap();
            let identity_key_path = temp_dir.join("ark/gyan/.ark/identity.key");
    ...
    fn create_account_rejects_invalid_addresses() {
        in_test_dir("ark_create_account_test", |temp_dir| {
            ...
            for b in bad {
                let err = create_account(temp_dir, b).err().expect("expected error");
    ...
    fn create_account_rejects_duplicate() {
        in_test_dir("ark_create_account_test", |temp_dir| {
            create_account(temp_dir, "gyan@example.com").unwrap();
            let err = create_account(temp_dir, "gyan@example.com").err().expect("expected error");
