...
mod tests {
    ...
    fn cmd_delete_removes_file() {
        ...
        create_account_with_key(&td.0, &address, &[250u8; 32]).unwrap();
    ...
    fn cmd_delete_removes_directory_recursively() {
        ...
        let port = start_test_server(td.0.clone());
        let address = format!("gyan@127.0.0.1:{}", port);
        create_account_with_key(&td.0, &address, &[251u8; 32]).unwrap();
    ...
    fn cmd_delete_missing_file_errors() {
        ...
        let port = start_test_server(td.0.clone());
        let address = format!("gyan@127.0.0.1:{}", port);
        create_account_with_key(&td.0, &address, &[252u8; 32]).unwrap();
    ...
    fn cmd_delete_via_address_form() {
        ...
        let port = start_test_server(td.0.clone());
        let address = format!("gyan@127.0.0.1:{}", port);
        create_account_with_key(&td.0, &address, &[253u8; 32]).unwrap();
